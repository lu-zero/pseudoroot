#!/usr/bin/env bash
# Compare pseudoroot vs fakeroot vs fakeroost performance
...
for cmd in "$HELPER" "$PSEUDOROOT"; do
    if [ ! -x "$cmd" ]; then
        echo "Error: $cmd does not exist or is not executable"
        exit 1
    fi
done

# fakeroot is available at /usr/bin/fakeroot
if ! command -v fakeroot >/dev/null 2>&1; then
    echo "Warning: fakeroot not found, skipping fakeroot comparison"
    NO_FAKEROOT=true
else
    NO_FAKEROOT=false
fi

# Check if fakeroost exists
if [ ! -x "$FAKEROOST" ]; then
    echo "Warning: fakeroost not found at $FAKEROOST, skipping fakeroost comparison"
    NO_FAKEROOST=true
else
    NO_FAKEROOST=false
fi

echo "=== pseudoroot vs fakeroot vs fakeroost Performance Comparison ==="
...
run_fakeroost() {
    local workers=$1
    if [ "$NO_FAKEROOST" = true ]; then
        echo "N/A"
        return
    fi
    # fakeroost uses a different approach - we need to use it as a library
    # For now, we'll use the CLI approach similar to fakeroot
    # Note: fakeroost might need special handling
    $FAKEROOST $HELPER $N_FAKE $workers $WORKDIR 2>&1 | grep -o 'rate=[0-9.]*' | cut -d= -f2 || echo "N/A"
}

# Print header
if [ "$NO_FAKEROOT" = false ] && [ "$NO_FAKEROOST" = false ]; then
    echo "Benchmark Results:"
    echo ""
    printf "%10s %15s %15s %15s %15s\n" "Workers" "Native (stats/s)" "pseudoroot (stats/s)" "fakeroot (stats/s)" "fakeroost (stats/s)"
    printf "%10s %15s %15s %15s %15s\n" "--------" "-------------" "-----------------" "----------------" "------------------"
elif [ "$NO_FAKEROOT" = false ]; then
    echo "Benchmark Results:"
    echo ""
...
for workers in "${WORKERS[@]}"; do
    ...
    if [ "$NO_FAKEROOST" = false ]; then
        fakeroost_rate=$(run_fakeroost $workers)
        printf "%10d %15.0f %15.0f %15.0f %15s\n" $workers $native_rate $pseudo_rate $fake_rate $fakeroost_rate
    elif [ "$NO_FAKEROOT" = false ]; then
...
if [ "$NO_FAKEROOST" = false ]; then
    BASELINE_FAKEROOST=$(run_fakeroost 1)
fi
...
if [ "$NO_FAKEROOST" = false ]; then
    printf "  fakeroost:   %15.0f stats/s\n" $BASELINE_FAKEROOST
fi
echo ""
...
if [ "$NO_FAKEROOST" = false ]; then
    FAKEROOST_OVERHEAD=$(echo "scale=1; ($BASELINE_NATIVE - $BASELINE_FAKEROOST) * 100 / $BASELINE_NATIVE" | bc)
    printf "  fakeroost:   %15.1f%%\n" $FAKEROOST_OVERHEAD
fi
echo ""

# Efficiency comparison
if [ "$NO_FAKEROOT" = false ]; then
...
if [ "$NO_FAKEROOST" = false ]; then
    if (( $(echo "$PSEUDO_OVERHEAD < $FAKEROOST_OVERHEAD" | bc -l) )); then
        IMPROVEMENT=$(echo "scale=1; ($FAKEROOST_OVERHEAD - $PSEUDO_OVERHEAD)" | bc)
        echo "✅ pseudoroot is $IMPROVEMENT% MORE EFFICIENT than fakeroost"
    else
        SLOWER=$(echo "scale=1; ($PSEUDO_OVERHEAD - $FAKEROOST_OVERHEAD)" | bc)
        echo "❌ pseudoroot is $SLOWER% SLOWER than fakeroost"
    fi
fi
...
if [ "$NO_FAKEROOT" = false ] && [ "$NO_FAKEROOST" = false ]; then
    printf "%10s %15s %15s %15s %15s\n" "Workers" "Native x1" "pseudoroot x1" "fakeroot x1" "fakeroost x1"
    printf "%10s %15s %15s %15s %15s\n" "--------" "----------" "--------------" "------------" "--------------"
elif [ "$NO_FAKEROOT" = false ]; then
...
for workers in "${WORKERS[@]}"; do
    ...
    if [ "$NO_FAKEROOST" = false ]; then
        fakeroost_rate=$(run_fakeroost $workers)
        fakeroost_speedup=$(echo "scale=2; $fakeroost_rate / $BASELINE_FAKEROOST" | bc)
        printf "%10d %15.2fx %15.2fx %15.2fx %15.2fx\n" $workers $native_speedup $pseudo_speedup $fake_speedup $fakeroost_speedup
    elif [ "$NO_FAKEROOT" = false ]; then
